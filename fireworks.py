...
launched_fireworks = 0
exploded_fireworks = 0
class Firework:
    
    def __init__(self):
        global launched_fireworks
        mixer.Channel(launched_fireworks).play(mixer.Sound("launch.mp3"))
        if (launched_fireworks > 6):
            launched_fireworks = 0
        else:
            launched_fireworks = launched_fireworks + 1

        self.colour = tuple(randint(100, 255) for _ in range(3))
        self.colours = tuple(tuple(randint(100, 255) for _ in range(3)) for _ in range(3))
    ...
    def explode(self):
        global exploded_fireworks
        # when the firework has entered a stand still, create the explosion particles
        mixer.Channel(exploded_fireworks).play(mixer.Sound("explode.mp3"))
        mixer.Channel(exploded_fireworks).play(mixer.Sound("spread.mp3"))
        if (exploded_fireworks > 6):
            exploded_fireworks = 0
        else:
            exploded_fireworks = exploded_fireworks + 1
